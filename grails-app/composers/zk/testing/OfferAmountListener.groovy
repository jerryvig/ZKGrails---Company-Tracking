...
public class OfferAmountListener extends ColumnListener { 
  def fullDateFormatter = new SimpleDateFormat("MM/dd/yyyy")

  def OfferAmountListener( def _tableRows ) {
    tableRows = _tableRows
    sortDirection = "DESC"
    query = "SELECT * FROM (SELECT type, company_name, company_url, ticker_symbol_ii, offer_amount, date_priced, nasdaq_profile_url, city, state FROM nasdaq_ipos WHERE state='CA' ORDER BY offer_amount "+sortDirection+") where rownum<251"
  }
