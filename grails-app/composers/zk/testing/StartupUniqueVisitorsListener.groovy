...
public class StartupUniqueVisitorsListener extends ColumnListener { 
  def fullDateFormatter = new SimpleDateFormat("MM/dd/yyyy")

  def StartupUniqueVisitorsListener( def _tableRows ) {
    tableRows = _tableRows
    sortDirection = "DESC"
    query = "SELECT * FROM (SELECT * FROM second_market_fact_table WHERE ( state='CA' AND unique_visitors>0 ) ORDER BY unique_visitors "+sortDirection+") where rownum<251"
  }
