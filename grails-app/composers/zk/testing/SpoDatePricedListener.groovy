...
public class SpoDatePricedListener extends ColumnListener { 
  def fullDateFormatter = new SimpleDateFormat("MM/dd/yyyy")

  def SpoDatePricedListener( def _tableRows ) {
    tableRows = _tableRows
    sortDirection = "DESC"
    query = "SELECT * FROM (SELECT type, company_name, company_url, ticker_symbol, offer_amount, date_priced, nasdaq_profile_url, city, state FROM nasdaq_spos WHERE state='CA' ORDER BY date_priced "+sortDirection+") where rownum<251"
  }
