...
public class SharpeListener extends ColumnListener { 

  def SharpeListener( def _tableRows ) {
    tableRows = _tableRows
    sortDirection = "DESC"
    query = "SELECT * FROM (SELECT * FROM quantcast_report ORDER BY sharpe_growth "+sortDirection+") WHERE rownum<251"
  }
